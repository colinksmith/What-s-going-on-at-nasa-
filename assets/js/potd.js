...
potdObj.updatePageInfo = function(){
    ...
    potdDate.textContent = this.getReadableDate(this.data.date)
    headerDate.textContent = this.getReadableDate(this.data.date)
...
potdObj.getReadableDate = function(dataDate){
    let output = dataDate.split('-')
    return ([output[1],output[2],output[0]].join('-'))
}
